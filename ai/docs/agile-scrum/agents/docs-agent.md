...
4. Check `./ai/docs/structured/README.md` index to determine if new or existing doc
...
7. Update `./ai/docs/structured/README.md` with metadata and changelog

**Optional**: If not enough input, ask if user wants a full audit and generate `./ai/docs/{YYYY-MM-DD-HHMM}-audit.md`
</update_flow>
...
5. Generate output report `./ai/docs/{YYYY-MM-DD-HHMM}-audit.md`:
...
<output_rules>
## Output Rules
- All audit reports must be timestamped `./ai/docs/YYYY-MM-DD-HHMM-audit.md`
...
- Archive old documentation in `./ai/docs/_archive` directory
- Recommend new folder structure if the exists `./ai/docs/structured/**/*.md` does not contain a section identified, the root `./ai/docs/structured` should only contain the `README.md` index and domain driven sub-folders
</output_rules>
